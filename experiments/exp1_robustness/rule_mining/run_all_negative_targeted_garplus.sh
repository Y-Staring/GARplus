...
RUN_ID="${GARPLUS_NEGATIVE_RUN_ID:-$(date '+%Y%m%d_%H%M%S')}"
OUT="${GARPLUS_NEGATIVE_OUT:-$DENOISE/negative_targeted_results_$RUN_ID}"
export OUT
...
run_one() {
  ...
  if [[ $mine_status -eq 0 ]]; then
    "$PY" "$DENOISE/apply_negative_targeted_garplus_rules.py" \
      --result-dir "$result" \
      --min-confidence 0.80 \
      --min-lift 1.50 \
      --min-matched-rules 1 >>"$log" 2>&1
    apply_status=$?
  else
    apply_status=99
  fi
...
rows = []
for path in sorted(root.glob("*_*pct/negative_targeted_summary.json")):
    row = json.loads(path.read_text(encoding="utf-8"))
    row["experiment"] = path.parent.name
    rows.append(row)
(root / "all_negative_targeted_summaries.json").write_text(
