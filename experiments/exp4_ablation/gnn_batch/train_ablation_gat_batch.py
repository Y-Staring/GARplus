...
def evaluate(model: nn.Module, loader: DataLoader, graph: dgl.DGLGraph, feat: torch.Tensor, device: torch.device) -> dict[str, float]:
    ...
    fp_positive = int(cm[0, 1] + cm[2, 1])
    fp_label2_as_positive = int(cm[2, 1])
    positive_tn = int(cm[0, 0] + cm[0, 2] + cm[2, 0] + cm[2, 2])
    positive_fpr = fp_positive / (fp_positive + positive_tn) if (fp_positive + positive_tn) else 0.0
    metrics.update(
        label2_recall=float(per_class_rec[2]),
        label2_precision=float(per_class_pre[2]),
        fp_positive=float(fp_positive),
        fp_label2_as_positive=float(fp_label2_as_positive),
        positive_fpr=float(positive_fpr),
        cm_00=float(cm[0, 0]),
        cm_01=float(cm[0, 1]),
        cm_02=float(cm[0, 2]),
        cm_10=float(cm[1, 0]),
...
def train_model(
    ...
    for epoch in range(args.epochs):
        ...
        print(
            ...
            f"FP+={int(metrics['fp_positive'])} | FPR+={metrics['positive_fpr']:.4f}",
            flush=True,
        )
