...
class GarRunConfig:
    ...
    use_sampled_pt_graph: bool = False
    ...
    include_ml_predicate_targets: bool = False
    ...
    inject_sampled_frequent_patterns: bool = False
    ...
    enable_predicate_bn: bool = False
    tau_x: float = 0.5
...
def run_demo(cfg: GarRunConfig) -> None:
    ...
    generated_total_before_filter = len(generated)
    ...
    if cfg.global_rematch_patterns and patterns_to_mine:
        # VSpawn may use capped parent embeddings for fast structural exploration.
        # Rebuild only the deduplicated and size-selected candidates globally before rule mining.
    ...
    print(
        f"[Patterns] generated_total={generated_total_before_filter} unique_total={len(unique_patterns)} "
        f"mining_total={len(patterns_to_mine)} "
        f"deduped={generated_total_before_filter - len(unique_patterns)} size_filtered={size_filtered_out} "
        f"min_pattern_nodes={cfg.min_pattern_nodes} max_pattern_nodes={cfg.max_pattern_nodes} "
    ...
    print(
        f"[Timing] stage=rule_mining_total algorithm=GAR dataset={cfg.dataset_name} "
