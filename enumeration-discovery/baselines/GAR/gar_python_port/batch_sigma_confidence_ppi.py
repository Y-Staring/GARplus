...
from gar_demo_runner import run_demo
...
RESULT_DIR = Path(__file__).resolve().parent / "batch_results" / "sigma_confidence_ppi"
...
def config_for_run(sigma: int, confidence: float):
    tag = f"s{sigma}_c{confidence_tag(confidence)}_n{PATTERN_SIZE}"
    return replace(
        CONFIG,
        ...
        deduped_rules_output_path=str(RESULT_DIR / f"deduped_rules_{tag}.txt"),
        pattern_instances_output_path=str(RESULT_DIR / f"pattern_instances_{tag}.jsonl"),
    )


def parse_result(log_text: str) -> tuple[str, str, str, str]:
    timing_match = TIMING_RE.search(log_text)
    patterns_match = PATTERNS_RE.search(log_text)
    rules_match = RULES_RE.search(log_text)
    return (
        timing_match.group(1) if timing_match else "",
        patterns_match.group(1) if patterns_match else "",
        rules_match.group(1) if rules_match else "",
        rules_match.group(2) if rules_match else "",
    )
...
def main() -> None:
    ...
    for varying_param, sigma, confidence in runs:
            ...
            rule_mining_seconds, patterns_mined, raw_rules, deduped_rules = parse_result(log_text)
            print(
                ...
                f"raw_rules={raw_rules or 'NA'} deduped_rules={deduped_rules or 'NA'} "
